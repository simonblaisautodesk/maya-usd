...
#define USE_NEW_BEHAVIOUR_FOR_ALL
#define USE_UUID_TO_FIND_REFNODE
//#define REUSE_REFNODE // 1 to many relation ship between MayaReference and Reference Node (Comment out for 1-1)
#define USE_MAYANAMESPACE_IN_REFNODE_NAME
...
{
#ifdef USE_NEW_BEHAVIOUR_FOR_ALL
...
MString UsdMayaTranslatorMayaReference::getUniqueRefNodeName(const UsdPrim& prim, const MFnDagNode& parentDag, const MFnReference& refDependNode)
{
    ...
    if(isMayaReference(prim))
    {
        ...
        // node.
        //
#ifdef USE_MAYANAMESPACE_IN_REFNODE_NAME
        uniqueRefNodeName = namespaceFromPrim(prim);
        if(!uniqueRefNodeName.isEmpty())
            uniqueRefNodeName += "_";
#endif
        ...
        uniqueRefNodeName += (fsFilePath.filename().wstring() + std::wstring(L"RN")).c_str();
    } else {
...
//  Always have to create the attribute, if we are here it because the prim didn't have it.
//    if(!prim.HasAttribute(MayaReferenceAttribute)) 
    ...
    #ifdef USE_UUID_TO_FIND_REFNODE
                ...
                if (isAttributeOnPrim && value.GetType() == SdfValueTypeNames->String.GetType()) {
                    ...
                    for (MItDependencyNodes refIter(MFn::kReference); !refIter.isDone();
                        ...
                        if (!tempRefFn.isFromReferencedFile()) {
                            ...
                            bool equal = false;
    #ifdef USE_UUID_TO_FIND_REFNODE
                            equal = (expectedValue == tempRefFn.uuid());
    #else
                            equal = (expectedValue == tempRefFn.name());
    #endif                        
                            if (equal) {                            
...
#ifdef REUSE_REFNODE                
            if (refNode.isNull()) {
                // Reuse existing Maya Reference Node with the same name
                for (MItDependencyNodes refIter(MFn::kReference); !refIter.isDone(); refIter.next()) {
                    MObject      tempRefNode = refIter.item();
                    MFnReference tempRefFn(tempRefNode);
                    if (!tempRefFn.isFromReferencedFile()) {
                        // What reference node name is the prim expecting?
                        MString expectedRefName = getUniqueRefNodeName(prim, parentDag, tempRefFn);
